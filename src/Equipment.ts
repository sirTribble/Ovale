/*////////////////////////////////////////////////////////////////////
    Copyright (C) 2011, 2012, 2016 Sidoine De Wispelaere.
    Copyright (C) 2012, 2013, 2014 Johnny C. Lam.
    See the file LICENSE.txt for copying permission.
//*/
import { select,  strgsub, strmatch, tonumber, tostring, type, unpack, wipe, CreateFrame, GetAuctionItemSubClasses, GetInventoryItemID,
		GetItemInfo, INVSLOT_AMMO, INVSLOT_BACK, INVSLOT_BODY, INVSLOT_CHEST, INVSLOT_FEET,
		INVSLOT_FINGER1, INVSLOT_FINGER2, INVSLOT_FIRST_EQUIPPED, INVSLOT_HAND, INVSLOT_HEAD, INVSLOT_LAST_EQUIPPED,
		 INVSLOT_LEGS, INVSLOT_MAINHAND,
		INVSLOT_NECK, INVSLOT_OFFHAND, INVSLOT_RANGED, INVSLOT_SHOULDER, INVSLOT_TABARD, INVSLOT_TRINKET1,
		INVSLOT_TRINKET2, INVSLOT_WAIST, INVSLOT_WRIST, ITEM_LEVEL, GetEquippedItemType, GetItemLevel,
	GetNormalizedWeaponSpeed, UIParent, len } from 'stub';
import { ovaleProfiler } from './Profiler';
import {ovale} from './Ovale';
import AceEvent from 'AceEvent-3.0';

// Frame for tooltip-scanning.
var self_tooltip = null
var OVALE_ITEM_LEVEL_PATTERN = "^" + strgsub(ITEM_LEVEL, "%%d", "(%%d+)")
...
var OVALE_ARMORSET_SLOT_IDS = { INVSLOT_CHEST, INVSLOT_HAND, INVSLOT_HEAD, INVSLOT_LEGS, INVSLOT_SHOULDER }
...
var OVALE_WEAPON_CLASS = {}
{
	...
	OVALE_WEAPON_CLASS[17] = GetAuctionItemSubClasses(1)	// "Fishing Poles"
}

var OVALE_META_GEM = null
{
	var _, _, _, _, _, _, name = GetAuctionItemSubClasses(8)	// "8" is "Gem"
	OVALE_META_GEM = name
}

// Normalized weapon attack speeds (http.//www.wowpedia.org/Normalization)
var OVALE_NORMALIZED_WEAPON_SPEED = {
...
class OvaleEquipment {
	...
	equippedItems:LuaDictionary<number> = {}
	// Item levels of equipped items, indexed by slot ID.
	equippedItemLevels:LuaDictionary<number> = {}
	// Type of main-hand item equipped.
	mainHandItemType = null
	// Type of off-hand item equipped.
	offHandItemType = null
	// Count of equipped pieces of an armor set. armorSetCount[armorSetName] = equippedCount
	armorSetCount = {}
	// Item ID of meta gem if equipped.
	metaGem = null

	// Normalized weapon speeds for equipped mainhand && offhand weapons.
	mainHandWeaponSpeed = null
	offHandWeaponSpeed = null
	
	profiler = ovaleProfiler.RegisterProfiling('Equipment');
	events = AceEvent.Embed({});

	//<private-static-methods>
	GetEquippedItemType(slotId) {
		this.profiler.StartProfiling("OvaleEquipment_GetEquippedItemType")
		var itemId = this.GetEquippedItem(slotId)
		var itemType: string;
		if ( itemId ) {
			var itemInfo = GetItemInfo(itemId);
			itemType = itemInfo.itemEquipLoc
		}
		this.profiler.StopProfiling("OvaleEquipment_GetEquippedItemType")
		return itemType
	}

	GetItemLevel(slotId) {
		this.profiler.StartProfiling("OvaleEquipment_GetItemLevel")
		self_tooltip.SetInventoryItem("player", slotId)
		var itemLevel
		for (let i = 2 ; i <=  self_tooltip.NumLines(); i++) {
			var text = _G["OvaleEquipment_ScanningTooltipTextLeft" + i].GetText()
			if ( text ) {
		...
		this.profiler.StopProfiling("OvaleEquipment_GetItemLevel")
		return itemLevel
	}

	GetNormalizedWeaponSpeed(slotId) {
		this.profiler.StartProfiling("OvaleEquipment_GetNormalizedWeaponSpeed")
		var weaponSpeed
		if ( slotId == INVSLOT_MAINHAND || slotId == INVSLOT_OFFHAND ) {
			var itemId = this.GetEquippedItem(slotId)
			if ( itemId ) {
				var _, _, _, _, _, _, weaponClass = GetItemInfo(itemId)
				// weaponSpeed = OVALE_NORMALIZED_WEAPON_SPEED[weaponClass]
			}
		}
		this.profiler.StopProfiling("OvaleEquipment_GetNormalizedWeaponSpeed")
		return weaponSpeed
	}
	//</private-static-methods>
	...
	OnInitialize() {
		...
		self_tooltip = CreateFrame("GameTooltip", "OvaleEquipment_ScanningTooltip", null, "GameTooltipTemplate")
		self_tooltip.SetOwner(UIParent, "ANCHOR_NONE")
	}

	OnEnable() {
		this.events.RegisterEvent("GET_ITEM_INFO_RECEIVED", event => this.GET_ITEM_INFO_RECEIVED(event));
		this.events.RegisterEvent("PLAYER_ENTERING_WORLD", event => this.UpdateEquippedItems())
		this.events.RegisterEvent("PLAYER_AVG_ITEM_LEVEL_UPDATE", event => this.UpdateEquippedItemLevels())
		this.events.RegisterEvent("PLAYER_EQUIPMENT_CHANGED", (event, slotId, hasItem) => this.PLAYER_EQUIPMENT_CHANGED(event, slotId, hasItem))
	}

	OnDisable() {
		this.events.UnregisterEvent("GET_ITEM_INFO_RECEIVED")
		this.events.UnregisterEvent("PLAYER_ENTERING_WORLD")
		this.events.UnregisterEvent("PLAYER_AVG_ITEM_LEVEL_UPDATE")
		this.events.UnregisterEvent("PLAYER_EQUIPMENT_CHANGED")
	}

	GET_ITEM_INFO_RECEIVED(event) {
		this.profiler.StartProfiling("OvaleEquipment_GET_ITEM_INFO_RECEIVED")
		...
		if ( changed ) {
			...
			this.events.SendMessage("Ovale_EquipmentChanged")
		}
		this.profiler.StopProfiling("OvaleEquipment_GET_ITEM_INFO_RECEIVED")
	}
	...
	PLAYER_EQUIPMENT_CHANGED(event, slotId, hasItem) {
		...
		if ( hasItem ) {
			...
			this.equippedItems[slotId] = null
			this.equippedItemLevels[slotId] = null
			if ( slotId == INVSLOT_MAINHAND ) {
				this.mainHandItemType = null
			} else if ( slotId == INVSLOT_OFFHAND ) {
				this.offHandItemType = null
			}
		}
	...
	GetArmorSetCount(name) {
		var count = this.armorSetCount[name]
		if ( !count ) {
			var className = ovale.playerClass
	...
	GetEquippedItem(slotId:string|number) {
	...
	GetEquippedItemLevel(slotId:number|string) {
		if ( slotId && typeof(slotId) !== "number" ) {
			slotId = OVALE_SLOTNAME[slotId]
		}
		if ( slotId ) {
			return this.equippedItemLevels[slotId]
		} else {
			return null
		}
	}
	...
	HasEquippedItem(itemId: number, slotId?:number|string) {
		if (slotId) {
			...
			for (let slotId in this.equippedItems) {
				let equippedItemId = this.equippedItems[slotId];
	...
	HasMainHandWeapon(handedness?:number) {
		if ( handedness ) {
			if ( handedness == 1 ) {
				return this.mainHandItemType == "INVTYPE_WEAPON"
					|| this.mainHandItemType == "INVTYPE_WEAPONMAINHAND"
	...
	HasOffHandWeapon(handedness?:number) {
		if ( handedness ) {
			if ( handedness == 1 ) {
				return this.offHandItemType == "INVTYPE_WEAPON"
					|| this.offHandItemType == "INVTYPE_WEAPONOFFHAND"
					|| this.offHandItemType == "INVTYPE_WEAPONMAINHAND"
			} else if ( handedness == 2 ) {
	...
	HasTrinket(itemId) {
		return this.HasEquippedItem(itemId, INVSLOT_TRINKET1)
			|| this.HasEquippedItem(itemId, INVSLOT_TRINKET2)
	}
	...
	HasOneHandedWeapon(slotId) {
		...
		if ( slotId ) {
			...
			return this.mainHandItemType == "INVTYPE_WEAPON" || this.mainHandItemType == "INVTYPE_WEAPONMAINHAND"
				|| this.offHandItemType == "INVTYPE_WEAPON" || this.offHandItemType == "INVTYPE_WEAPONMAINHAND"
		}
		return false
	}

	UpdateArmorSetCount() {
		this.profiler.StartProfiling("OvaleEquipment_UpdateArmorSetCount")
		wipe(this.armorSetCount)
		for (let i = 1 ; i <=  len(OVALE_ARMORSET_SLOT_IDS); i++) {
			var itemId = this.GetEquippedItem(OVALE_ARMORSET_SLOT_IDS[i])
			if ( itemId ) {
				var name = OVALE_ARMORSET[itemId]
				if ( name ) {
		...
		this.profiler.StopProfiling("OvaleEquipment_UpdateArmorSetCount")
	}
	...
	UpdateEquippedItems() {
		...
		var changed = false
		var item
		for (let slotId = INVSLOT_FIRST_EQUIPPED ; slotId <=  INVSLOT_LAST_EQUIPPED; slotId++) {
		...
		var changedItemLevels = this.UpdateEquippedItemLevels()
	...
	UpdateEquippedItemLevels() {
		this.profiler.StartProfiling("OvaleEquipment_UpdateEquippedItemLevels")
		var changed = false
		var itemLevel
		for (let slotId = INVSLOT_FIRST_EQUIPPED ; slotId <=  INVSLOT_LAST_EQUIPPED; slotId++) {
		...
		this.profiler.StopProfiling("OvaleEquipment_UpdateEquippedItemLevels")
		return changed
	}
	...
	DebugEquipment() {
		...
		for (let k in this.armorSetCount) {
			let v = this.armorSetCount[k];
			this.Print("Player has %d piece(s) of %s armor set.", v, k)
		}
	}

	Print(text:string, ...parameters) {}
}


export var ovaleEquipment = ovale.NewModule("OvaleEquipment", new OvaleEquipment(), "AceEvent-3.0")
